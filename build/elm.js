...
Elm.Sudoku.make = function (_elm) {
   "use strict";
   _elm.Sudoku = _elm.Sudoku || {};
   if (_elm.Sudoku.values) return _elm.Sudoku.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var RemovePossiblesFromSquare = {ctor: "RemovePossiblesFromSquare"};
   var view = F2(function (address,model) {
      var buttonLine = function (_p0) {
         var _p1 = _p0;
         return A2($Html.li,
         _U.list([]),
      ...
      _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("sudoku")]),A2($List.map,htmlRow,rows(model)))
              ...
              A2($List.map,
              buttonLine,
              _U.list([{ctor: "_Tuple2",_0: RemovePossiblesFromSquare,_1: "remove possibles from 3x3 squares"}
   ...
   var extractColumn = F2(function (c,m) {
      return _U.cmp(c,0) < 0 || _U.cmp(c,$Matrix.colCount(m)) > -1 ? _U.list([]) : A2($List.filterMap,
      function (rowNr) {
         return A2($Matrix.get,{ctor: "_Tuple2",_0: rowNr,_1: c},m);
      },
   ...
   var removePossiblesFromLines = function (model) {
      var columnValues = $Array.fromList(A2($List.map,
      function (colNr) {
         return filledInValues(A2(extractColumn,colNr,model));
      },
   ...
   var handleSingleOnLine = function (model) {
      ...
      rowsAndColumns);
      var singles = $Dict.fromList(A3($List.foldl,
      F2(function (vec,accu) {    return A2($Basics._op["++"],$Utils.findSingles(vec),accu);}),
      _U.list([]),
      unFilleds));
   ...
   return _elm.Sudoku.values = {_op: _op
                               ...
                               ,hard: hard
                               ,extreme: extreme
                               ,charListToModel: charListToModel
                               ,init: init
                               ,subMatrix: subMatrix
...
Elm.Main.make = function (_elm) {
   ...
   $Sudoku = Elm.Sudoku.make(_elm);
   var _op = {};
   var main = $StartApp$Simple.start({model: $Sudoku.init,update: $Sudoku.update,view: $Sudoku.view});
